...
if ((Get-Process|Where-Object{$_.MainModule.ModuleName -like "*$ProcessName*"}).length -gt 0) {
    ...
    Write-Host ("xperf -stop HeapLeak -stop 'NT Kernel Logger' -d '{1}\{0}_Heap-Trace.etl'" -f $ProcessName, $Env:TEMP)
    ...
    Write-Host ("To stop tracing in PowerShell, run: wpr.exe -stop '{1}\{0}_Heap-Trace.etl' ;wpr.exe -HeapTracingConfig {0} disable" -f $ProcessName, $Env:TEMP)
    Write-Host ("To stop tracing in CMD, run: wpr.exe -stop '{1}\{0}_Heap-Trace.etl' &wpr.exe -HeapTracingConfig {0} disable" -f $ProcessName, $Env:TEMP)
}
